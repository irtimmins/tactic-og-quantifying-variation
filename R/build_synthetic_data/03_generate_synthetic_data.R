...
library(here)
syn_dir <- here("Data", "synthetic")    # reads profile from here, writes synth here
dir.create(syn_dir, recursive = TRUE, showWarnings = FALSE)
profile <- readRDS(file.path(syn_dir, "og_profile_for_synthetic.rds"))
set.seed(20260601)
...
saveRDS(syn_cohort_out, file.path(syn_dir, "og_cohort_precwt_SYNTH.rds"))
...
saveRDS(syn_cwt, file.path(syn_dir, "cwt_records_SYNTH.rds"))
...
write_dta(syn_cohort_out_stata, file.path(syn_dir, "og_cohort_precwt_SYNTH.dta"))
write_dta(syn_cwt, file.path(syn_dir, "cwt_records_SYNTH.dta"))